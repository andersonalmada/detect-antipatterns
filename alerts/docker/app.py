...
def detect_time_window(groups, window_seconds=60):
    ...
    que compartilham o mesmo host, service, name e severity,
    ...
    for group in groups:
        ...
        for alert in group["alerts"]:
            key = (alert["host"], alert["service"], alert["name"], alert["severity"])
            subgroups[key].append(alert)

        # Verificação de redundância temporal por subgrupo
        for (_, _, _, _), alerts in subgroups.items():
...
def detector():
    ...
    grouped = detect_time_window(grouped, window_seconds=60)
